...
$id_sesion = $_SESSION['usuario']; // ID del usuario logueado
...
if (isset($_POST['accion'])) {
    ...
    if ($accion === 'agregar' || $accion === 'editar') {
        ...
        // ---------- VALIDACIONES ----------
        if (
            $nombre === '' || $dni === '' || $telefono === '' || $correo === '' || $estado === '' ||
...
if (isset($_GET['ajax']) && $_GET['ajax'] == 'tabla') {
    ...
    echo "<tr data-estado='{$data_estado}'>
            ...
            <td class='acciones'>
              ...
              <button class='btn-accion btn-eliminar' onclick=\"eliminarEmpleado({$id_row})\">
                🗑️ Eliminar
              </button>
            </td>
          </tr>";
...
<div class="module-container">
  <div class="module-header">
    <div class="header-content">
      ...
      <div class="header-text">
        ...
        <p>Administra empleados del sistema</p>
      </div>
    </div>
  </div>
  ...
  <div class="module-toolbar">
    ...
    <div class="toolbar-right">
      <button class="btn-secondary"
              style="padding:10px 18px; border-radius:6px; cursor:pointer; margin-right:10px;"
              onclick="abrirReporteGeneralEmpleados()">
          <span class="btn-icon">📊</span> Generar Reporte
      </button>
    </div>
  </div> <!-- cierre .module-toolbar -->
  ...
  <!-- MODAL PARA REPORTES (GENERAL E INDIVIDUAL) -->
  <div class="report-overlay" id="overlayReporteEmpleados">
    <div class="report-modal">
      <div class="report-modal-header">
        <span class="report-close" onclick="cerrarReporteEmpleados()">&times;</span>
      </div>
      <iframe id="iframeReporteEmpleados" src="" frameborder="0"></iframe>
    </div>
  </div>
...
<style>
  ...
  /* HEADER MÁS PARECIDO A LOS OTROS MÓDULOS */
  .module-header {
    background: #ffffff;
    color: #000000;
    padding: 20px;
...
.btn-accion {
  ...
  padding: 4px 8px;      /* menos ancho y alto */
  border-radius: 16px;
  font-weight: 600;
  display: inline-flex;
  align-items: center;
...
/* Botón Editar (amarillo negro, similar al de Usuarios) */
.btn-editar {
  background: #ffc107;
  color: #000;
}
...
async function cargarTabla(){
  try{
    ...
    document.getElementById('tablaEmpleados').innerHTML =
      `<div class="error-msg">No se pudo cargar la tabla de empleados (${err.message}).</div>`;
  }
}
...
window.onclick = function(event) {
  ...
  const rep   = document.getElementById('overlayReporteEmpleados');
  if (event.target === modal) cerrarModal();
  if (event.target === rep) cerrarReporteEmpleados();
};
...
function abrirReporteGeneralEmpleados(){
  const overlay = document.getElementById('overlayReporteEmpleados');
  const iframe  = document.getElementById('iframeReporteEmpleados');
  iframe.src    = '/modulos/reporte_general_empleados.php';
...
function verReporteEmpleado(id){
  const overlay = document.getElementById('overlayReporteEmpleados');
  const iframe  = document.getElementById('iframeReporteEmpleados');
  iframe.src    = '/modulos/reporte_individual.php?id='+id+'&modulo=empleados';
  overlay.style.display = 'flex';
}

function cerrarReporteEmpleados(){
  const overlay = document.getElementById('overlayReporteEmpleados');
  const iframe  = document.getElementById('iframeReporteEmpleados');
  iframe.src = '';
